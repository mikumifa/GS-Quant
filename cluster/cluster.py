...
from utils import (
    compute_clusters_embeddings,
    find_leaves,
    find_nearest_keys_lca_based,
    label_,
    map_child_to_parent,
    node2parentpath,
    refine_1,
    ...
    refine_3,
    refine_4,
    rename_clusters_to_ints,
    rename_unique_keys,
    sort_entity_embeddings,
)

with open("./openai_api.key", "r") as f:
    api_key = f.read().strip()
base_url = "http://172.29.7.148:11434/v1"
...
def generate_entity_description(entity, hint=None):
    ...
    response = gpt_chat_return_response(model=chat_model, prompt=prompt)
...
def generate_embeddings(args, entity_info, entity_embeddings, dim=1024):
    ...
    original_descriptions = [
        entity_info[entity]["original_description"] for entity in entities
    ]
    ...
    if os.path.exists(f"{args.output_dir}/{args.dataset}/entity_init_embeddings.json"):
        print(
            f"Loading existing entity embeddings from {args.output_dir}/{args.dataset}/entity_init_embeddings.json..."
        )
        with open(
            f"{args.output_dir}/{args.dataset}/entity_init_embeddings.json", "r"
        ) as f:
        ...
        if all(entity_embeddings[entity] is not None for entity in entities):
            ...
            print(
                f"Loading existing entity info from {args.output_dir}/{args.dataset}/entity_info.json..."
            )
            with open(f"{args.output_dir}/{args.dataset}/entity_info.json", "r") as f:
                entity_info = json.load(f)
            print("Done.")
            return (
                np.array([entity_embeddings[entity] for entity in entities]),
                entity_info,
                entity_embeddings,
            )
    ...
    def process_entity(i):
        ...
        if entity_embeddings[entity] is None:
            ...
            combined_embedding = np.concatenate(
                (entity_embedding, description_embedding)
            )
    ...
    with ThreadPoolExecutor(max_workers=args.num_threads) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(entities)):
            ...
            if futures.index(future) == len(entities) - 1:
                with open(
                    f"{args.output_dir}/{args.dataset}/entity_info.json", "w"
                ) as f:
                    json.dump(entity_info, f, indent=4)

                with open(
                    f"{args.output_dir}/{args.dataset}/entity_init_embeddings.json", "w"
                ) as f:
...
def perform_or_load_clustering(args, entities, embeddings, distance_threshold):
    ...
    if not os.path.exists(clustering_file):
        print(
            f"Performing agglomerative clustering with distance threshold {distance_threshold:.2f}..."
        )
...
def agglomerative_clustering(args, entities, embeddings, distance_threshold):
    ...
    """
    clustering = perform_or_load_clustering(
        args, entities, embeddings, distance_threshold
    )
...
def seed_hierarchy_construction(args, entities, embeddings, distance_threshold):
    ...
    """
    clusters, clustering = agglomerative_clustering(
        args, entities, embeddings, distance_threshold
    )
    # clustering is the AgglomerativeClustering object
    # clusters is a dictionary mapping cluster IDs to lists of entity labels
    # Get the root of the hierarchy
    initial_hierarchy = build_hierarchy(
        clustering.children_, len(entities), entities, clustering
    )
    # Build the hierarchy tree
    seed_hierarchy = build_seed_hierarchy(clusters, initial_hierarchy)

    seed_hierarchy = {"Cluster_top": seed_hierarchy}
...
def evaluate_threshold(args, entities, embeddings, threshold):
    ...
    if num_clusters == 1:
        logging.info(
            f"Skipping evaluation for threshold {threshold:.2f}: Only one cluster formed."
        )
        return threshold, -1.0, None
    elif num_clusters == len(entities):
        logging.info(
            f"Skipping evaluation for threshold {threshold:.2f}: Each entity is in its own cluster."
        )
...
def find_optimal_threshold(
    args, entities, embeddings, min_threshold=0.1, max_threshold=1.0, num_thresholds=10
):
    ...
    logging.info(
        f"Starting threshold evaluation with {num_thresholds} thresholds: {thresholds}"
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        futures = [
            executor.submit(evaluate_threshold, args, entities, embeddings, threshold)
    ...
    if best_threshold is None:
        logging.info(
            "No suitable clustering found. Returning the clustering with the highest threshold."
        )
        best_threshold = thresholds[-1]
        best_clusters = agglomerative_clustering(
            args, entities, embeddings, best_threshold
        )
        ...
        for entity in entities:
            if entity not in ori_info:
                ...
                entity_info[entity] = {
                    ...
                    "original_description": ori_info[entity]["description"]
                    if "description" in ori_info[entity]
                    else None,
                    "llm_description": None,
                }
...
def construct_args():
    parser = argparse.ArgumentParser(
        description="Cluster entities using hierarchical clustering and refine the clusters using LLM."
    )
    ...
    parser.add_argument(
        ...
        default="seed",
        choices=["seed", "llm"],
        help="Type of hierarchy to construct. Default: seed.",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="FB15K-237",
        help="Path to the dataset file containing the list of entities to cluster.",
    )
    parser.add_argument(
        "--dimensions",
        type=int,
        default=1024,
        help="Dimensionality of the embeddings. Default: 1024.",
    )
    parser.add_argument(
        "--num_threads",
        type=int,
        default=10,
        help="Number of threads to use for multi-threaded processes. Default: 10.",
    )
    parser.add_argument(
        "--max_entities",
        type=int,
        default=100,
        help="Maximum number of entities to include in an LLM request. Default: 100.",
    )
...
def main():
    ...
    embeddings, entity_info, entity_embeddings = generate_embeddings(
        args,
    ...
    if not os.path.exists(f"{args.output_dir}/{args.dataset}/seed_hierarchy.json"):
        ...
        best_threshold, best_clusters = find_optimal_threshold(
            args,
            entities_text,
            embeddings,
            min_threshold=0.05,
        ...
        seed_hierarchy = seed_hierarchy_construction(
            args, entities_text, embeddings, best_threshold
        )
        ...
        print(
            f"Loading existing seed clusters from {args.output_dir}/{args.dataset}/seed_hierarchy.json..."
        )
        with open(f"{args.output_dir}/{args.dataset}/seed_hierarchy.json", "r") as f:
            seed_hierarchy = json.load(f)

    seed_hierarchy_int, _, key_map, key_map_inv = rename_clusters_to_ints(
        seed_hierarchy
    )
    print("Done.")
    ...
    if not os.path.exists(
        f"{args.output_dir}/{args.dataset}/clusters_embeddings_seed.json"
    ):
        label2entity = {
            entity_info[entity]["text_label"]: entity for entity in entity_info.keys()
        }
        clusters_embeddings = compute_clusters_embeddings(
            clusters=seed_hierarchy,
            entity_embeddings=entity_embeddings,
        ...
        with open(
            f"{args.output_dir}/{args.dataset}/clusters_embeddings_seed.json", "w"
        ) as f:
    ...
    if not os.path.exists(
        f"{args.output_dir}/{args.dataset}/entity_info_seed_hier.json"
    ):
        entity_info_seed_hier = labeling_hierarchy_to_entities(
            seed_hierarchy_int, entity_info, num_threads=args.num_threads
        )
        with open(
            f"{args.output_dir}/{args.dataset}/entity_info_seed_hier.json", "w"
        ) as f:
            json.dump(entity_info_seed_hier, f, indent=4)
    print("Done.")

    if not os.path.exists(
        f"{args.output_dir}/{args.dataset}/entity_init_embeddings.npy"
    ):
        ...
        sorted_entity_embeddings = sort_entity_embeddings(
            entity_embeddings_dict=entity_embeddings, entity2id=entity2id
        )
        # save the sorted entity embeddings
        np.save(
            f"{args.output_dir}/{args.dataset}/entity_init_embeddings.npy",
            sorted_entity_embeddings,
        )

    if not os.path.exists(f"{args.output_dir}/{args.dataset}/llm_hierarchy.json"):
        print(
            "Please first use llm_refine.py to refine the seed hierarchy using LLM, and generate llm_hierarchy.json."
        )
    else:
        print(
            f"Loading existing llm clusters from {args.output_dir}/{args.dataset}/llm_hierarchy.json..."
        )
        ...
        llm_hierarchy_int, _, key_map, key_map_inv = rename_clusters_to_ints(
            llm_hierarchy
        )
        os.makedirs(f"{args.output_dir}/{args.dataset}/tmp", exist_ok=True)
        with open(
            f"{args.output_dir}/{args.dataset}/tmp/llm_hierarchy_int.json", "w"
        ) as f:
        ...
        print("Start Computing Clusters Embeddings...")
        if not os.path.exists(
            f"{args.output_dir}/{args.dataset}/clusters_embeddings_llm.json"
        ):
            label2entity = {
                entity_info[entity]["text_label"]: entity
                for entity in entity_info.keys()
            }
            clusters_embeddings = compute_clusters_embeddings(
                clusters=llm_hierarchy,
                entity_embeddings=entity_embeddings,
                label2entity=label2entity,
            )
            for cluster_id in clusters_embeddings:
                clusters_embeddings[cluster_id] = clusters_embeddings[
                    cluster_id
                ].tolist()
            with open(
                f"{args.output_dir}/{args.dataset}/clusters_embeddings_llm.json", "w"
            ) as f:
                json.dump(clusters_embeddings, f, indent=4)
            ...
            embs = np.array(embs)
            np.save(
                f"{args.output_dir}/{args.dataset}/clusters_embeddings_llm.npy", embs
            )
        print("Done.")

        print("Start labeling hierarchy information to entities...")
        if not os.path.exists(
            f"{args.output_dir}/{args.dataset}/entity_info_llm_hier.json"
        ):
            entity_info_llm_hier = labeling_hierarchy_to_entities(
                llm_hierarchy_int, entity_info
            )
            with open(
                f"{args.output_dir}/{args.dataset}/entity_info_llm_hier.json", "w"
            ) as f:
